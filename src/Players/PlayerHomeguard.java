 ...
 */
public class PlayerHomeguard extends Player {

    public PlayerHomeguard(Color color, int playerNumber, int turnTime, String name) {
        super(color, playerNumber, turnTime, name);
    }

    public PlayerHomeguard(Color color, int playerNumber, int turnTime) {
        super(color, playerNumber, turnTime, "Homeguard");
    }
                ...
                //bases don't need defending by this ant
                    ...
                    for (Point p : antVision) {
                        if (enemyHills.containsKey(p)) {
                            for (Point destination : legalMoves) {
                                if (BoardGenerator.manhattanDistance(destination, p) < BoardGenerator.manhattanDistance(bestMove, p)) {
                                    bestMove = destination;
                                } else if (BoardGenerator.manhattanDistance(destination, p) == BoardGenerator.manhattanDistance(bestMove, p)) {
                                    if (BoardGenerator.euclideanDistance(destination, p) < BoardGenerator.euclideanDistance(bestMove, p)) {
                                        bestMove = destination;
                                    }
                                }
                            }
