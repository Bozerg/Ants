 ...
 */
public class PlayerPathfinder extends Player {
    private final HashMap<Point, Hill> knownEnemyHills = new HashMap<Point, Hill>();
    private HashMap<Point, Hill> myPreviousHills;
    private final HashSet<Point> knownRocks = new HashSet<Point>();
    private final Random r = new Random();
    private HashSet<Ant> ants;
    private HashSet<Ant> enemyAnts;
    private Board visibleBoard;
    private HashMap<Point, HashSet<Point>> visible;
    private HashMap<Point, Food> food;
    private HashMap<Point, Hill> enemyHills;
    private HashSet<Hill> enemyHillSet;
    private HashSet<Hill> myHills;
    private HashSet<Rock> rocks;
    private HashMap<Point, Hill> myHillMap;

    public PlayerPathfinder(Color color, int playerNumber, int turnTime, String name) {
        super(color, playerNumber, turnTime, name);
    }

    public PlayerPathfinder(Color color, int playerNumber, int turnTime) {
        super(color, playerNumber, turnTime, "Pathfinder");
    }

    public Integer getPathDistance(Point a, Point b) {
        if (a.equals(b)) {
            return 0;
        }
        ...
        HashMap<Point, Integer> distanceMap = new HashMap<Point, Integer>();
        Point current = a;
        ...
        while (!frontier.isEmpty()) {
            ...
            Point[] potentialMoves = new Point[4];
            potentialMoves[0] = new Point((int) ((current.getX() - 1 + width) % width), (int) current.getY());
            potentialMoves[1] = new Point((int) ((current.getX() + 1 + width) % width), (int) current.getY());
            potentialMoves[2] = new Point((int) current.getX(), (int) ((current.getY() - 1 + height) % height));
            potentialMoves[3] = new Point((int) current.getX(), (int) ((current.getY() + 1 + height) % height));
            for (Point p : potentialMoves) {
                if (p.equals(b)) {
                    return distanceMap.get(current) + 1;
                }
                if (!distanceMap.containsKey(p) && !explored.contains(p) && !knownRocks.contains(p)) {
                    frontier.add(p);
                    distanceMap.put(p, distanceMap.get(current) + 1);
                }
            }
        }
        return null;
    ...
    @Override
    public void run() {
        ants = currentState.getMyAnts();
        enemyAnts = currentState.getEnemyAnts();
        visibleBoard = currentState.getVisibleBoard();
        visible = visibleBoard.getVisibleInfoForPlayer(getColor());
        food = currentState.getFoodLocation();
        enemyHills = currentState.getEnemyHillLocation();
        enemyHillSet = currentState.getEnemyHills();
        myHills = currentState.getMyHills();
        rocks = currentState.getRocks();
        int kamikazeCount = 0;
        for (Rock rock : rocks) {
            knownRocks.add(rock.getLocation());
        }
        ...
        myHillMap = currentState.getMyHillLocation();
        ...
        while (!turnOver) {
            ...
            if (visibleBoard.getTurnCount() > 30) {
            ...
            for (Ant enemyAnt : enemyAnts) {
                ...
                for (Hill hill : myHills) {
                    if (BoardGenerator.manhattanDistance(enemyAnt.getLocation(), hill.getLocation()) < 4) {
                        for (Ant myAnt : ants) {
            ...
            for (Ant ant : ants) {
                ...
                //bases don't need defending by this ant
                    ...
                    int minPathDistance = height + width;
                    for (Point p : antVision) {
                        if (enemyHills.containsKey(p)) {
                            hillFlag = true;
                            for (Point destination : legalMoves) {
                                int currentPathDistance = getPathDistance(destination, p);
                                if (currentPathDistance < minPathDistance) {
                                    bestMove = destination;
                                    minPathDistance = currentPathDistance;
                                }
                            }
                        }
                    }
                    ...
                    if (bestMove.equals(curLocation) && !foodFlag && !splitFlag && !hillFlag) {
                        ...
                        if (r.nextInt(4) == 0) {
                            ...
                            if (knownEnemyHills.keySet().size() == 0 || ants.size() < myHills.size() * 10 || kamikazeCount >= ants.size() / 2.5) {
                                for (Point destination : legalMoves) {
                                    if (!curLocation.equals(destination) && !myHillMap.containsKey(destination)) {
                                        ...
                                        for (Ant a : ants) {
                                            ...
                                            if (!other.equals(curLocation)) {
                                                int distanceToOther = getPathDistance(destination, other);
                                ...
                                minPathDistance = width * height;
                                for (Point p : knownEnemyHills.keySet()) {
                                    for (Point destination : legalMoves) {
                                        if (BoardGenerator.manhattanDistance(destination, p) < 20 || (r.nextInt(3) == 0 && BoardGenerator.manhattanDistance(destination, p) < 35)) {
                                            int currentPathDistance = getPathDistance(destination, p);
                                            if (currentPathDistance < minPathDistance) {
                                                bestMove = destination;
                                                minPathDistance = currentPathDistance;
